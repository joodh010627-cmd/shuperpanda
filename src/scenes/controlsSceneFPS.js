...
export class ControlsSceneFPS {
  ...
  render(ctx) {
    ...
    const boxY = CANVAS_H / 2 - 30;
    ctx.fillStyle = 'rgba(0, 0, 0, 0.6)';
    ctx.fillRect(100, boxY - 50, CANVAS_W - 200, 220);
    ctx.strokeStyle = '#fbbf24';
    ctx.lineWidth = 2;
    ctx.strokeRect(100, boxY - 50, CANVAS_W - 200, 220);

    ctx.font = '20px "Press Start 2P"';
    ctx.fillStyle = '#fff';
    ctx.fillText('미션: 커피 더 헛을 무찌르세요!', CANVAS_W / 2, boxY + 20);
    
    ctx.font = '16px "Press Start 2P"';
    ctx.fillStyle = '#ff5555';
    ctx.fillText('※ 주의: 미니 커피 더 헛이 있을 때', CANVAS_W / 2, boxY + 80);
    ctx.fillText('보스는 무적(IMMUNE) 상태입니다.', CANVAS_W / 2, boxY + 110);

    // Controls
    ctx.font = '18px "Press Start 2P"';
    ctx.fillStyle = '#fbbf24';
    ctx.fillText('- 조작 방법 -', CANVAS_W / 2, boxY + 180);
    ctx.font = '16px "Press Start 2P"';
    ctx.fillStyle = '#fff';
    ctx.fillText('마우스로 조준하고 클릭하여 사격하세요!', CANVAS_W / 2, boxY + 220);
